...
function copySelectedId() { const id = state.row?.id ?? state.selectedEntry?.id ?? ''; if (globalThis.navigator?.clipboard && id) globalThis.navigator.clipboard.writeText(id).catch(() => {}); document.getElementById('copy-id-button').textContent = id ? 'ID copied' : 'No ID selected'; }
