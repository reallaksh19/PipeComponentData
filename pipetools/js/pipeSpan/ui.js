...
export function renderPipeSpanInputs(host, state, actions) {
  ...
  const sizes = uniqueNps.map((nps) => `<option ${Number(nps) === Number(input.nps) ? 'selected' : ''}>${nps}</option>`);
  const schedules = getPipeSpanSchedules(input.nps).map((schedule) =>
    `<option ${schedule === input.schedule ? 'selected' : ''}>${esc(schedule)}</option>`);
  ...
  for (const id of ['service', 'insulation', 'material', 'beamMethod']) document.getElementById(`span-${id}`).value = input[id];
...
export function renderPipeSpanMain(state, pipeSpanSvg) {
  ...
  document.getElementById('table-count').textContent = `NPS ${result.input.nps}`;
...
function resultTable(result) {
  const rows = [
    ...
    ['Selected method span', result.selectedMethodSpanM, 'm'],
    ['Least allowable span', result.leastAllowableSpanM, 'm'],
...
function summary(result) {
  return `${kv('Service', result.input.service)}${kv('Insulation', result.input.insulation)}${kv('Schedule', result.input.schedule)}${kv('Selected span', `${result.selectedMethodSpanM} m`)}${kv('Least allowable', `${result.leastAllowableSpanM} m`)}${kv('Governing span', `${result.governingSpanM} m`)}${kv('QMS ref.', `${result.qmsReferenceM ?? '—'} m`)}`;
}
