...
test('DB Phase 5: normalized pipe schedule inventory is pinned to source tables', () => {
  ...
  assert.equal(pipes.summary.sampledRowCount, 489);
  ...
  assert.equal(pipes.sourceFiles['Database/Pipe/PIPE80.csv'], 24);
});
...
test('DB Phase 5: pipe index matches normalized rows and has no duplicate keys', () => {
  ...
  assert.equal(pipeIndex.readyKeys.length, 384);
  assert.equal(pipeIndex.partialKeys.length, 105);
