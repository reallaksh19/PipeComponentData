...
test('DB Phase 14: indexed wave 5 rows resolve to normalized catalogs', () => {
  ...
  assert.equal(result.families.FLANGE.indexedRows, 533);
  assert.equal(result.families.VALVE.indexedRows, 898);
  assert.equal(result.families.FITTING.indexedRows, 3898);
...
test('DB Phase 14: source coverage exposes wave 5 sampled and blocked families', () => {
  ...
  assert.equal(result.families.FLANGE.sourceCoverage.sampledRowCount, 683);
  assert.equal(result.families.VALVE.sourceCoverage.sampledRowCount, 1275);
  assert.equal(result.families.FITTING.sourceCoverage.sampledRowCount, 3898);
