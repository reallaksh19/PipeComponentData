...
test('DB Phase 45: bounded SCH80 addenda are promoted without bulk schedule import', () => {
  ...
  if (!fs.existsSync('data/normalized/pipes-expanded.json')) {
    ...
    assert.ok(indexedPipeRows.length >= 18);
  }
  assert.equal(indexedPipeRows.some((entry) => entry.id === 'PIPE|NPS10|SCH40'), true);
  ...
  assert.equal(allRows.some((row) => String(row.schedule) === '160'), true);
});
