...
test('DB Phase 26: valve catalog promotes bounded source-backed samples only', () => {
  ...
  assert.equal(valves.metadata.generationMode, 'SOURCE_BACKED_PROMOTION');
  assert.equal(valves.metadata.sampledRowCount, 599);
...
test('DB Phase 26: promoted Class 150 and 1500 gate valve values are pinned to source rows', () => {
  ...
  assert.equal(gate1500.sourceRowNumber, 11);
...
test('DB Phase 26: valve lookup remains exact and no nearest-class fallback is allowed', () => {
  ...
  const miss = componentSearch('gate valve 4 3000 rf', searchIndex, {
    aliases, mode: SEARCH_MODE.EXACT_ALIAS_ONLY,
    filters: { componentType: 'VALVE', valveType: 'GATE', nps: '4', classRating: '3000', facing: 'RF' },
  });
