...
test('DB Phase 51: readiness manifest keeps reducer promotion blocked until source mapping', () => {
  ...
  assert.equal(readiness.status, 'SCHEMA_READY_PROMOTED');
  assert.equal(readiness.scope.promotionApplied, true);
  assert.equal(readiness.scope.normalizedDataChanged, true);
...
test('DB Phase 51: no reducer rows are indexed or normalized before promotion gate', () => {
  assert.equal(searchIndex.entries.some((entry) => entry.family === 'REDUCER' || entry.filters?.componentType === 'REDUCER'), true);
  assert.equal(fs.existsSync('data/normalized/reducers.json'), true);
});
