...
test('DB Phase 44: valve catalog boundary is now fully promoted', () => {
  assert.equal(ledger.families.VALVE.latestPromotionPhase, 'DB_PHASE_76');
  assert.equal(valves.rows.length, preflight.currentBoundary.normalizedRowsRemain);
  assert.deepEqual(unique('valveType'), ['BALL', 'CONTROL', 'GATE', 'GLOBE']);
  assert.deepEqual(unique('endType'), ['FLANGED']);
  assert.deepEqual(unique('facing'), ['RF']);
  assert.deepEqual(unique('classRating'), ['150', '1500', '2500', '300', '400', '600', '900']);
});
