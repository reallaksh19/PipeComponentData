...
test('DB Phase 6: committed wave catalog is indexed and source-rooted to docs/Pipedata', () => {
  ...
  assert.equal(dataset.summary.explodedRowCount, 322);
  assert.equal(dataset.summary.generationMode, 'SOURCE_BACKED_PROMOTION');
  assert.equal(dataset.summary.sampledRowCount, 276);
