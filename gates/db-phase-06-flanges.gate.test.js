...
test('DB Phase 6: metric Flan source files are present and explode deterministically', () => {
...
test('DB Phase 6: 4 inch Class 300 WN flange values match source exactly', () => {
...
test('DB Phase 6: one Flan row explodes into WN, SO, and BLIND without losing subtype weights', () => {
  const rows = parsedMetricSources().flatMap(explodeFlangeRows);
...
test('DB Phase 6: committed sample catalog is indexed and source-rooted to docs/Pipedata', () => {
  ...
  assert.equal(dataset.summary.generationMode, 'SOURCE_TABLE_IN_REPO_EXPLODED_SAMPLE_ROWS');
