...
test('DB Phase 8: committed fitting wave catalog is source-backed and indexed', () => {
  ...
  assert.equal(catalog.metadata.phase, 'DB_PHASE_87');
  assert.equal(catalog.metadata.generationMode, 'SOURCE_BACKED_PROMOTION');
  assert.equal(catalog.metadata.sampledRowCount, 3898);
  assert.equal(catalog.rows.length, 3898);
  assert.equal(index.metadata.rowCount, 3898);
...
test('DB Phase 8: fitting exact catalog lookup stays key-based', () => {
  ...
  const miss = lookupFittingRecord(catalog.rows, { subtype: 'CAP', nps: '6', schedule: '400', unitSystem: 'METRIC' });
