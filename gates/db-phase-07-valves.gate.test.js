...
test('DB Phase 7: VLV1150 source table parses exact gate-valve dimensions', () => {
  ...
  assert.equal(gate8.dimensions.heightMm.value, 960);
  assert.equal(gate8.dimensions.handwheelDiaMm.value, 350);
  assert.equal(gate8.weights.rfRtjKg.value, 144);
  assert.equal(gate8.weights.buttWeldKg.value, 122);
});
...
test('DB Phase 7: N/A source valve values remain unavailable and never become zero', () => {
  ...
  assert.equal(gate22.dimensions.faceToFaceRtjMm.value, null);
...
test('DB Phase 7: committed valve sample catalog is source-backed and indexed', () => {
  ...
  const hit = lookupValveRecord(catalog.rows, {
    valveType: 'GATE', endType: 'FLANGED', nps: '8', classRating: '150', facing: 'RF',
  });
  assert.equal(hit.ok, true);
  assert.equal(hit.row.weights.rfRtjKg.value, 144);
});

test('DB Phase 7: valve catalog modules and gate stay under 200 lines', () => {
