...
test('DB Phase 68: exact lookup resolves wave 2 reducers and rejects wrong schedule', () => {
  const found = lookupComponentExact('REDUCER 6 4 SCH80', assets, { filters: { componentType: 'REDUCER', largeNps: '6', smallNps: '4', largeSchedule: '80' } });
  assert.equal(found.status, LOOKUP_STATUS.FOUND);
  // It matches the first one in the list or has specific resolution
  assert.ok(found.row.id.startsWith('REDUCER|'));
  assert.ok(found.row.id.endsWith('|NPS6|NPS4|SCH80'));
