...
test('DB Phase 53: bounded Class 600 WN/SO/BLIND rows are promoted through addenda', () => {
  ...
  assert.equal(index.entries.filter((entry) => entry.family === 'FLANGE').length, 533);
  ...
  const hasExpandedFlanges = fs.existsSync('data/normalized/flanges-expanded.json');
  assert.equal(index.entries.some((entry) => entry.id === 'FLANGE|WN|NPS12|CL600|METRIC'), hasExpandedFlanges);
});
