...
test('DB Phase 23: source-backed promotion candidates are explicit', () => {
  assert.deepEqual(promotionCandidates(ledger).map((item) => item.family).sort(), ['FITTING', 'FLANGE', 'OLET', 'PIPE', 'REDUCER', 'VALVE']);
  assert.deepEqual(familiesByExpansionStatus(ledger, 'READY_FOR_PROMOTION'), ['FITTING', 'FLANGE', 'OLET', 'PIPE', 'REDUCER', 'VALVE']);
  ...
  assert.equal(ledger.families.REDUCER.promotionPhase, 'DB_PHASE_64');
  assert.equal(ledger.families.OLET.promotionPhase, 'DB_PHASE_78');
});
...
test('DB Phase 23: blocked and manual-review families are not promoted', () => {
  ...
  assert.equal(ledger.families.OLET.status, 'READY_FOR_PROMOTION');
