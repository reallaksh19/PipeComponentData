...
test('DB Phase 52: bounded SCH80 rows are promoted through addenda without bulk import', () => {
  ...
  const pipeCount = index.entries.filter((entry) => entry.family === 'PIPE').length;
  ...
  const hasWave4 = fs.existsSync('data/normalized/pipes-sch80-wave4.json');
  assert.equal(index.entries.some((entry) => entry.id === 'PIPE|NPS20|SCH80'), hasWave4);
});
