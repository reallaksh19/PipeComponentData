...
test('DB Phase 28: expanded index entries resolve to catalogs', () => {
  ...
  assert.equal(searchIndex.entries.filter((entry) => entry.family === 'FLANGE').length, 275);
  assert.equal(searchIndex.entries.filter((entry) => entry.family === 'VALVE').length, 418);
...
test('DB Phase 28: source ledger keeps non-source families blocked or under review', () => {
  ...
  assert.ok(['DB_PHASE_55', 'DB_PHASE_58', 'DB_PHASE_67', 'DB_PHASE_69'].includes(ledger.families.PIPE.latestPromotionPhase));
  assert.ok(['DB_PHASE_56', 'DB_PHASE_59', 'DB_PHASE_74'].includes(ledger.families.FLANGE.latestPromotionPhase));
  assert.ok(['DB_PHASE_49', 'DB_PHASE_76'].includes(ledger.families.VALVE.latestPromotionPhase));
