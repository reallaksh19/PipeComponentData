...
test('DB Phase 27: BW fitting catalog promotes bounded source-backed samples only', () => {
  assert.equal(fittings.metadata.phase, 'DB_PHASE_87');
  assert.equal(fittings.metadata.generationMode, 'SOURCE_BACKED_PROMOTION');
  assert.equal(fittings.metadata.sampledRowCount, 3898);
...
test('DB Phase 27: promoted elbow, tee, cap and wave rows are source-backed', () => {
  ...
  assert.ok(fittings.rows.every((row) => row.dataStatus === 'READY' || row.dataStatus === 'PARTIAL'));
});
...
test('DB Phase 27: fitting lookup remains exact and no nearest-schedule fallback is allowed', () => {
  ...
  const miss = componentSearch('ftbw cap 6 sch400', searchIndex, {
    aliases, mode: SEARCH_MODE.EXACT_ALIAS_ONLY,
    filters: { componentType: 'FITTING', subtype: 'CAP', nps: '6', schedule: '400' },
  });
