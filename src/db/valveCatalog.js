...
const normHeader = (h) => {
  if (h === 'RTJ-F/F') return 'RTJ F/F';
  if (h === 'RTJ Addition') return 'RTJ Add Len';
  if (h === 'RTJ Gap') return 'gap';
  return h;
};

export function parseValveTable(text, options = {}) {
  const rows = splitCsvRows(text);
  const headers = (rows[1] ?? []).map(normHeader);
...
function makeValveRow(raw, options) {
  ...
  const heightVal = raw['Height-HW'] !== undefined ? raw['Height-HW'] : raw['Height'];
  const heightCol = raw['Height-HW'] !== undefined ? 'Height-HW' : 'Height';
  ...
  const row = {
    ...
    dimensions: {
      ...
      heightMm: taggedNumber(heightVal, heightCol),
  ...
  if (raw['BW F/F Short Pattern'] !== undefined) {
    row.dimensions.buttWeldShortMm = taggedNumber(raw['BW F/F Short Pattern'], 'BW F/F Short Pattern');
  }
  row.id = valveKey(row);
