...
function makeControlValveRow(raw, options) {
  ...
  const isInchRating = Number(classRating) >= 300;
  const base = {
    componentType: 'VALVE',
    valveType: 'CONTROL',
    endType: 'FLANGED',
  ...
  const row = {
    ...
    dimensions: {
      faceToFaceRfMm: taggedNumber(raw['RF Face to Face'], 'RF Face to Face', isInchRating),
      faceToFaceRtjMm: taggedNumber(raw['RTJ Face to Face'], 'RTJ Face to Face', isInchRating),
      heightMm: taggedNumber(raw['Height'], 'Height', isInchRating),
      diaphragmDiaMm: taggedNumber(raw['Diaphram Diameter'], 'Diaphram Diameter', isInchRating),
      sideHandwheelStandoutMm: taggedNumber(raw['Side HW Standout'], 'Side HW Standout', isInchRating),
      sideHandwheelDiaMm: taggedNumber(raw['Side HW Diameter'], 'Side HW Diameter', isInchRating),
    },
    weights: {
      rfRtjKg: taggedNumber(raw['weight RF/RTJ'], 'weight RF/RTJ', isInchRating),
      buttWeldKg: taggedNumber(raw['weight BW'], 'weight BW', isInchRating),
    },
    provenance: {
...
function taggedNumber(value, sourceColumn, isInchRating) {
  let parsed = numberOrNull(value);
  if (parsed !== null && isInchRating && !sourceColumn.includes('weight')) {
    parsed = parsed * 25.4;
  }
  return parsed === null
